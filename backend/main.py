...
from integrations import IntegrationClients
...
async def check_url(
    ...
    """
    **Link Shield** — runs URL through DB + SafeBrowsing + VirusTotal in parallel.
    """
    import asyncio
    db_result, sb_result, vt_result = await asyncio.gather(
        db.lookup_url(req.url),
        integrations.safebrowsing.check(req.url),
        integrations.virustotal.scan_url(req.url),
    )
    ...
    signals = _make_input_from_db(
        ...
        extra={
            "url_flagged_external": req.url_flagged_external or sb_result.is_malicious or vt_result.is_malicious,
            "url_is_phishing":      req.url_is_phishing or sb_result.is_phishing or vt_result.is_phishing,
        },
    )
    result = _scorer.score(signals)
    logger.info("check-url %s → %s (score=%d) [sb=%s vt=%s]",
                req.url, result.action, result.score,
                sb_result.is_malicious, vt_result.is_malicious)
    return result
