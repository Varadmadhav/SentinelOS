...
const API = "http://localhost:8000/api/screen";
...
export function ScreenShield() {
  ...
  // FIX: useCallback ensures stable reference; always receives the text to analyze
  // so we never rely on potentially stale screenText state
  const handleAnalyze = useCallback(async (inputText: string) => {
    const trimmed = inputText.trim();
    if (!trimmed) return;

    console.log("Analyzing text:", trimmed); // debug — remove after confirming it works
    ...
    try {
      const res = await axios.post<ScreenResult>(`${API}/analyze`, {
        screen_text: trimmed,
      });

      console.log("Response received:", res.data); // debug — remove after confirming it works
  ...
  // FIX: loadDemo passes text directly — no state dependency
  ...
  // FIX: manual button passes current screenText explicitly
  const handleManualAnalyze = () => {
    handleAnalyze(screenText);
  };

  const rc = result ? (riskColors[result.risk_level] ?? riskColors.SAFE) : null;
  const RIcon = result?.risk_level === "DANGEROUS"
    ? ShieldOff
        ...
        <div className="px-4 py-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {DEMO_SCREENS.map((demo, i) => (
                <button
                  ...
                  disabled={loading}
                  className="p-2.5 border border-gray-200 rounded-lg text-xs text-gray-700 text-left hover:bg-gray-50 active:bg-gray-100 disabled:opacity-50"
                >
                  {demo.label}
                </button>
              ))}
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm">
            ...
            <button
              ...
              className="w-full bg-orange-500 text-white py-3 rounded-lg text-sm font-medium disabled:opacity-50 transition-opacity"
            >
                        ...
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span
                            className={`text-xs px-2 py-0.5 rounded-full ${
                              severityColor[pattern.severity] ?? "bg-gray-100 text-gray-600"
                            }`}
                          >
                            {pattern.severity}
                          </span>
          ...
          <div className="bg-white rounded-xl p-4 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 bg-gray-50 rounded-lg">
                ...
                <p className="text-xs text-gray-500">Last scan time</p>
              </div>
              <div className="p-3 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-2 mb-1">
